...
for interval in range(num_intervals):
    ...
    for t in range(timesteps):
        for i in range(len(heights) - 1):
            ...
            wind_sq_h1, wind_sq_h2 = wind_square[t, i], wind_square[t, i + 1]
            wind_cu_h1, wind_cu_h2 = wind_cube[t, i], wind_cube[t, i + 1]
            
            # 插值计算 wind_interp, wind_square_interp, wind_cube_interp
            model.addConstr(
            ...
            model.addConstr(
                wind_square_interp[t] >= wind_sq_h1 + (height_vars[t] - h1) / (h2 - h1) * (wind_sq_h2 - wind_sq_h1) - (1 - select[t, i]) * 1e6
            )
            model.addConstr(
                wind_square_interp[t] <= wind_sq_h1 + (height_vars[t] - h1) / (h2 - h1) * (wind_sq_h2 - wind_sq_h1) + (1 - select[t, i]) * 1e6
            )
            model.addConstr(
                wind_cube_interp[t] >= wind_cu_h1 + (height_vars[t] - h1) / (h2 - h1) * (wind_cu_h2 - wind_cu_h1) - (1 - select[t, i]) * 1e6
            )
            model.addConstr(
                wind_cube_interp[t] <= wind_cu_h1 + (height_vars[t] - h1) / (h2 - h1) * (wind_cu_h2 - wind_cu_h1) + (1 - select[t, i]) * 1e6
            )
    ...
    total_energy = gp.quicksum(
        power_generation(wind_cube_min[t]) - energy_consumption(t, height_vars, height_vars, wind_square_interp[t], height_diff)
        for t in range(timesteps)
    )
    ...
    energy_contributions = [
        round(power_generation(wind_cube_min[t].X) - energy_consumption(t, height_vars, height_vars, wind_square_interp[t].X, height_diff).getValue(), 2)
        for t in range(timesteps)
    ]
